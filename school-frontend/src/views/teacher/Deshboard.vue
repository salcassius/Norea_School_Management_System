<template>
  <div class="font-[Battambang] p-6 min-h-screen bg-slate-50/50">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <div v-for="stat in stats" :key="stat.title" class="p-6 bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div v-else-if="schedule.length > 0" class="space-y-4">
          <div v-for="sched in schedule" :key="sched.id" class="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-xl transition-colors border border-transparent hover:border-slate-100">
            <div class="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-2 rounded-lg min-w-[80px] text-center">
              {{ sched.time }}
            </div>
            <div class="flex-1">
              <p class="text-sm font-bold text-slate-700">{{ sched.subject }}</p>
              <p class="text-[11px] text-slate-400">{{ sched.class_name }} • បន្ទប់ {{ sched.room }}</p>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div class="space-y-3">
          <button @click="$router.push('/teacher/attendance')" class="w-full text-left p-4 rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50 transition-all text-sm font-medium text-slate-600 flex items-center gap-2">
