    ...
    <div v-if="isOpen" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div class="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in zoom-in duration-150">
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <div>
            <h2 class="text-lg font-bold text-slate-800">កែសម្រួលឆ្នាំសិក្សា</h2>
            <p class="text-xs text-slate-500 mt-0.5">សូមកែប្រែព័ត៌មានឆ្នាំសិក្សាខាងក្រោម</p>
          </div>
        ...
        <form @submit.prevent="handleSubmit" class="p-5 space-y-4 text-sm">
          ...
          <label v-if="!props.editData?.is_active" class="flex items-center gap-2.5 cursor-pointer group py-1">
            <input 
...
watch(() => props.isOpen, (newVal) => {
  if (newVal && props.editData) {
    form.value = {
      year_name: props.editData.year_name,
      // កាត់យកតែទម្រង់ YYYY-MM-DD ដើម្បីឱ្យ Input type="date" បង្ហាញតម្លៃបានត្រឹមត្រូវ
...
const handleSubmit = async () => {
  loading.value = true
  try {
    await api.put(`/years/${props.editData.id}`, form.value)
    emit('success')
    closeModal()
  } catch (error) {
    console.error("Error detail:", error.response)
    alert("មានបញ្ហាក្នុងការកែសម្រួលទិន្នន័យ!")
  } finally {
    loading.value = false
  }
}
