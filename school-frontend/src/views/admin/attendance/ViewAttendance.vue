<template>
  <div class="font-[Battambang] p-3 sm:p-6 min-h-screen bg-slate-50/50 text-slate-700">
    <!-- Filter bar -->
    <div class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-300/80 shadow-sm flex flex-wrap items-end gap-4 mb-6">
      <div class="w-full sm:w-[220px]">
        <label class="text-[13px] sm:text-[14px] font-bold text-slate-600 uppercase">ថ្នាក់រៀន</label>
      ...
      <div class="w-full sm:w-[220px]">
        <label class="text-[13px] sm:text-[14px] font-bold text-slate-600 uppercase">ឆ្នាំសិក្សា</label>
    ...
    <!-- Export Buttons -->
    <div class="w-full flex flex-wrap justify-end gap-2 my-4">
      <button @click="exportExcel" :disabled="reportData.length === 0"
        class="flex-1 sm:flex-none bg-white border border-green-600 text-green-700 px-5 py-2.5 rounded-xl font-bold hover:bg-slate-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">
        នាំចេញ Excel
      </button>
      <button @click="exportPDF" :disabled="reportData.length === 0"
        class="flex-1 sm:flex-none bg-white border border-red-600 text-red-700 px-5 py-2.5 rounded-xl font-bold hover:bg-slate-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">
        នាំចេញ PDF
      </button>
    </div>
